...
------------------------'''
#forest.append(id3(training_data, index_to_values))
num_trees=25
num_records = len(training_data)
num_features = len(index_to_values)

for i in range(0, num_trees):
    bagsize = random.randint(1, num_records)
    spacesize = random.randint(1, num_features)
    
    #print('{0:2d} {1:3d} {2:4d}'.format(x, x*x, x*x*x))
    print("Adding tree with {0:2d} records and {1:3d} features".format( bagsize, spacesize) )
    forest.append(id3(get_bag(bagsize), get_random_subspace(spacesize)))
